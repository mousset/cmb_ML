...
for i in range(len(os.listdir(path_dir_data))):
    ...
    if all_cl_theo_binned.shape[0] >= nbmodels:
        break
        """
        for i in range(len(os.listdir("data_qsub_1000"))):
            data_filename = "data_qsub_1000/data_file_partial_sky_{}_nside_{}_{}.pickle".format(spectrum, nside, i)
            data_file = open(data_filename, "rb")
            [all_cl_theo_trans, all_alm_ana_trans, all_cl_namaster_trans] = pickle.load(data_file)
            all_cl_theo = np.append(all_cl_theo, all_cl_theo_trans, axis=0)
            all_cl_theo_trans = []
            all_alm_ana = np.append(all_alm_ana, all_alm_ana_trans, axis=0)
            all_alm_ana_trans = []
            all_cl_namaster = np.append(all_cl_namaster, all_cl_namaster_trans, axis=0)
            all_cl_namaster_trans = []
            data_file.close()
...
if noisy_bool:
    all_cl_anafast_binned = all_cl_anafast_binned[:nbmodels_max]
...
"""
if spectrum == "TE":
     all_cl_namaster += noise_rms**2*4*np.pi/npix
"""
...
#all_alm_ana = np.stack((all_alm_ana.real/np.max(np.abs(all_alm_ana.real)), all_alm_ana.imag/np.max(np.abs(all_alm_ana.imag))), axis=-1)
...
#if nbmodels < ilim:
#    x_train = all_alm_ana[0:nbmodels, :]
#    y_train = all_cl_theo[0:nbmodels, :]
...
#if nbmodels < ilim:
#    sample_variance_train = sample_variance[0:nbmodels, :]
...
def custom_error_simple(sample_variance_train, data_gen):
    def loss(y_true, y_pred):
        if not K.is_tensor(y_pred):
            y_pred = K.constant(y_pred)
        y_true = K.cast(y_true, y_pred.dtype)
        # We need to select the corresponding sample variance
        selected_sample_variance_train = sample_variance_train[data_gen.current_batch_indices]
        
        # full sky case: y_true = mean(y_pred) for Anafast
        chi2_loss = K.sum(K.abs(y_pred - y_true)**2/selected_sample_variance_train, axis=-1)/(n_bins-1)

        error = chi2_loss + better_than_loss
        return error
    return loss

def custom_metric_simple(sample_variance_test, y_pred, y_true):

    # full sky case: y_true = mean(y_pred) for Anafast
    chi2_loss = np.sum(np.abs(y_pred - y_true)**2/sample_variance_test, axis=-1)/(n_bins-1)

    error = chi2_loss
    return error

"""Selecting corresponding functions"""

custom_error = custom_error_simple
custom_metric = custom_metric_simple

"""Build a model"""
nalm_model = 300#int(nl * (nl + 1) / 2)#200#500
print(nalm_model)
input_layer = Input(shape=(all_cl_namaster.shape[-1],))
...
#hidden = Dense(units=nalm_model*6, activation='relu')(hidden)
#hidden = Dense(units=nalm*6, activation='relu')(hidden)
if dropout_val > 0:
...
filename = "log_perf_PartialSky_from_anafast.txt"
f = open(filename, "a")
